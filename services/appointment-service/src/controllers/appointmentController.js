...
const { buildAppointmentPublicView, canAccess } = require("../services/appointmentHelpers");
...
function createAppointmentController({ appointmentSseBroadcaster }) {
  const searchDoctors = asyncHandler(async (req, res) => {
    ...
    const doctors = result.data?.data?.doctors || result.data?.data || result.data;
    return response.sendSuccess(res, { message: "doctors found", data: { doctors } });
  });
  ...
  const bookAppointment = asyncHandler(async (req, res) => {
    ...
    return response.sendSuccess(res, { statusCode: 201, message: "appointment booked", data: { appointment: buildAppointmentPublicView(appointment) } });
  });
  ...
  const cancelAppointment = asyncHandler(async (req, res) => {
    ...
    if (!canAccess({ user: req.user, appointment })) throw new AppError("Forbidden", 403);
    if (appointment.status === "cancelled") return response.sendSuccess(res, { message: "already cancelled", data: { appointmentId } });

    appointment.status = "cancelled";
    appointment.statusUpdatedAt = new Date();
    appointment.events.push({ type: "cancelled", detail: "Appointment cancelled" });
    ...
    return response.sendSuccess(res, { message: "appointment cancelled", data: { appointmentId } });
  });
  ...
  return { searchDoctors, bookAppointment, cancelAppointment, listMyAppointments, statusStream };
}
