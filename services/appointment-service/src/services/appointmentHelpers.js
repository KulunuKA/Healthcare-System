function buildAppointmentPublicView(appt, doctorData = null) {
  console.log(
    `Building appointment view for ${appt._id}, doctorData:`,
    doctorData,
  );
  return {
    id: appt._id,
...
function canAccess({ user, appointment }) {
  ...
  if (user.role === "patient")
    return String(appointment.patientId) === String(user.sub);
  if (user.role === "doctor")
    return String(appointment.doctorId) === String(user.sub);
  return false;
}
