...
async function getProfile(doctorId) {
  const doctor = await Doctor.findById(doctorId)
    .select(
      "_id email fullName specialty offerTelemedicine verified availability",
    )
    .lean();
...
async function setAvailability(doctorId, { slots }) {
  if (!Array.isArray(slots) || slots.length === 0)
    throw new AppError("slots[] is required", 400);
...
async function listDoctors({ specialty }) {
  ...
  const doctors = await Doctor.find(filter)
    .select("_id fullName specialty offerTelemedicine verified")
    .lean();
  return doctors.map((d) => ({
    id: d._id,
    fullName: d.fullName,
    specialty: d.specialty,
    offerTelemedicine: d.offerTelemedicine,
    verified: d.verified,
  }));
}
